...
def test_board_digital_write_with_bugged_and_clean_smbus(v_smbus):
    ...
    for io_name, gpio in board.Consts.IO.all_constants.items():
        assert board.gpio_digital_read(gpio) is False
        board.gpio_digital_write(gpio, False)
        assert board.gpio_digital_read(gpio) is False

        board.gpio_digital_write(gpio, True)
        assert board.gpio_digital_read(gpio) is True
...
def test_board_digital_write_all_with_bugged_and_clean(v_smbus):
    ...
    # should be all off == 0
    assert board.gpio_digital_read_all() == all_regs_off, "test if the default is off after init"

    board.gpio_digital_write_all(False)
    assert board.gpio_digital_read_all() == all_regs_off, "should be 0 if all set to false"

    # we set everything on
    board.gpio_digital_write_all(True)
    assert board.gpio_digital_read_all() == [
...
def test_random_gpio_toggle_and_state_consistency(v_smbus):
    ...
    assert board.gpio_digital_read_all() == expected_state
    ...
    for _ in range(100):
        ...
        board.gpio_digital_write(gpio, new_state)
        ...
        # Verify digital_read
        assert board.gpio_digital_read(gpio) == new_state, f"{io_name} read mismatch"

        # Verify digital_read_all
        assert board.gpio_digital_read_all() == expected_state, "Mismatch after toggling GPIO"
...
def test_multiple_gpio_toggle_patterns(v_smbus):
    ...
    for _ in range(5):
        ...
        assert board.gpio_digital_read_all() == expected_state, "State mismatch after setting random GPIOs ON"
        ...
        assert board.gpio_digital_read_all() == expected_state, "State mismatch after setting GPIOs OFF"
